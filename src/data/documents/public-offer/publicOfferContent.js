import { DocsTextList } from "../../../components/DocsTextList/DocsTextList";

export const point0EN = [
  {
    text: (
      <p>
        This Public Offer Agreement (hereinafter - the &#34;Offer&#34;,
        &#34;Agreement&#34;) defines the terms of access to the
        &#34;MusicDex&#34; project (hereinafter - the &#34;Platform&#34;),
        located on the website musicdex.co (hereinafter - the
        &#34;Website&#34;).
      </p>
    ),
  },
  {
    text: (
      <p>
        This contract is public, i.e. in accordance with Article 633 of the
        Civil Code of Ukraine, its terms are the same for all users regardless
        of status (individual, legal entity, individual entrepreneur) without
        favoring one user over another.
      </p>
    ),
  },
  {
    text: (
      <p>
        Full and unconditional acceptance of the terms of the Agreement, i.e.
        the Platform&#39;s public offer, is the fact of payment for Services on
        the Site. The User&#39;s registration on the Site is also considered
        full and unconditional acceptance of the terms of the Agreement.
      </p>
    ),
  },
  {
    text: (
      <p>
        By accepting the terms of the Agreement, the User confirms that he is
        fully and completely familiar with and agrees with its terms and
        conditions of use of the Platform, and, if the User is an individual,
        gives permission to process his personal data in order to be able to
        fulfill the terms of this Agreement, the possibility of mutual
        settlements, delivery, as well as to receive invoices, acts and other
        documents. Permission to process personal data is valid for the entire
        term of the Agreement. In addition, by entering into this Agreement, the
        User confirms that he has been notified (without additional notice) of
        the rights established by the Law of Ukraine &#34;On Personal Data
        Protection&#34;, of the purposes of data collection. The scope of the
        User&#39;s rights as a subject of personal data in accordance with the
        Law of Ukraine &#34;On Personal Data Protection&#34; is known and
        understood by him.
      </p>
    ),
  },
];

export const point0UA = [
  {
    text: (
      <p>
        Даний Договір публічної оферти (далі – «Оферта», «Договір») визначає
        умови доступу до проєкту «MusicDex» (далі – «Платформа»), розміщеного на
        веб-сайті musicdex.co (далі – «Веб-сайт»).
      </p>
    ),
  },
  {
    text: (
      <p>
        Цей договір є публічним, тобто відповідно до статті 633 Цивільного
        кодексу України, його умови є однаковими для всіх користувачів незалежно
        від статусу (фізична особа, юридична особа, фізична особа-підприємець)
        без надання переваги одному користувачеві перед іншим.
      </p>
    ),
  },
  {
    text: (
      <p>
        Повним і безумовним прийняттям умов Договору, тобто публічної оферти
        Платформи, є факт здійснення оплати Послуг на Сайті. Повним і безумовним
        прийняттям умов Договору також вважається реєстрація Користувача на
        Сайті.
      </p>
    ),
  },
  {
    text: (
      <p>
        Приймаючи умови Договір, Користувач підтверджує, що цілком і повністю
        ознайомлений та згоден з його умовами та Умовами використання Платформи,
        а також, у разі, якщо Користувач є фізичною особою, дає дозвіл на
        обробку своїх персональних даних з метою можливості виконання умов цього
        Договору, можливості проведення взаєморозрахунків, доставки, а також для
        отримання рахунків, актів та інших документів. Дозвіл на обробку
        персональних даних діє протягом усього строку дії Договору. Крім цього,
        укладанням цього Договору, Користувач підтверджує, що йому повідомлено
        (без додаткового повідомлення) про права, встановлені Законом України
        «Про захист персональних даних», про цілі збору даних. Обсяг прав
        Користувача, як суб&#39;єкта персональних даних відповідно до Закону
        України «Про захист персональних даних», йому відомий і зрозумілий.
      </p>
    ),
  },
];

export const pointsEN = [
  {
    title: <p>1. DEFINITION OF TERMS</p>,
    point: [
      {
        text: (
          <p>
            This clause defines the interpretation of the following terms and
            definitions used in this Offer.
          </p>
        ),
      },
      {
        text: (
          <p>
            «Website» – A web page located on a domain name: musicdex.co,
            including the totality of all information, texts, graphic elements,
            design elements, images, photos and videos and other results of
            intellectual activity, as well as information and technical means
            posted on the web page. The term &#34;Website&#34; includes links to
            other open or closed web resources used to ensure the operation of
            the web page;{" "}
          </p>
        ),
      },
      {
        text: (
          <p>
            «User» – any individual who has access to the Website via the
            Internet;
          </p>
        ),
      },
      {
        text: (
          <p>
            «NFT» – ( non-fungible token) - a unique digital asset in the form
            of a non-fungible token;
          </p>
        ),
      },
      {
        text: (
          <p>
            «Token» – A virtual asset that embodies ownership rights to a share
            of the profits from a piece of music.
          </p>
        ),
      },
      {
        text: (
          <p>
            «Services» – an opportunity for music fans (&#34;Investors&#34;) to
            contribute to the financing of the production of video and audio
            recordings, as well as record promotion and marketing activities,
            providing an opportunity for fans to purchase a share of the rights
            to participate in the net revenues (eligibility) of songs and/or
            recordings in the form of a Token.
          </p>
        ),
      },
      {
        text: (
          <p>
            All terms and definitions not defined in this section shall be
            interpreted in the meaning established by the current legislation of
            Ukraine, and in the absence of such a definition – in their usual
            sense.{" "}
          </p>
        ),
      },
    ],
  },

  {
    title: <p>2. INTRODUCTION</p>,
    point: [
      {
        text: (
          <p>
            This Offer, together with the Privacy and Personal Data Protection
            Policy and other information posted on the Website, determine the
            terms of Users&#39; access to the Website, as well as the terms of
            use of the Platform posted on the Website.
          </p>
        ),
      },
      {
        text: (
          <p>
            To obtain information on the procedure for receiving, processing and
            storing personal data, the User can familiarize himself with the
            Privacy and Personal Data Protection Policy posted on the Website.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>3. TERMS OF ACCEPTANCE OF THE OFFER</p>,
    point: [
      {
        text: (
          <p>
            The User&#39;s use of this Website means the User&#39;s automatic
            and full acceptance of the terms of this Offer.
          </p>
        ),
      },
      {
        text: (
          <p>
            If the User does not agree with the terms of this Offer, the latter
            is obliged to stop using the Website and immediately leave it.{" "}
          </p>
        ),
      },
      {
        text: (
          <p>
            By accepting the terms of this Offer, the User confirms that he is
            at least 18 years old.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>4. INTERACTIONS WITH THE PLATFORM</p>,
    point: [
      {
        text: (
          <p>
            The Platform provides Users with the opportunity to acquire
            ownership rights to part of the royalties from a musical work
            expressed in Tokens produced within the framework of the MusicDex
            project and transferred to Users by connecting it to the Platform
            using a cryptocurrency wallet.
          </p>
        ),
      },
      {
        text: (
          <p>
            All transactions between the Platform and Users are carried out
            exclusively on the Polygon blockchain network.{" "}
          </p>
        ),
      },
      {
        text: (
          <p>
            In the case of purchasing a Token, the User pays a transaction fee
            (gas) charged in the blockchain network.
          </p>
        ),
      },
      {
        text: (
          <p>
            The User shall bear all costs associated with the acquisition of the
            Token and the receipt of royalty profits, in particular, for all
            taxes arising and applicable to the User associated with such
            acquisition.
          </p>
        ),
      },
      {
        text: (
          <p>
            The User uses the Platform solely at his own risk. The Platform does
            not provide the User with any guarantees regarding its use and
            purchase of the Token.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>5. SUBJECT OF THE AGREEMENT</p>,
    point: [
      {
        text: (
          <p>
            The Platform undertakes to transfer the Service to the User&#39;s
            ownership, and the User undertakes to pay for and accept the
            Services under the terms of this Agreement.
          </p>
        ),
      },
      {
        text: (
          <p>
            This Agreement regulates the purchase and sale of services on the
            Website, including:
          </p>
        ),
      },
      {
        text: DocsTextList([
          "voluntary choice by the User of the type of goods (Tokens) on the website;",
          "independent placement of an order by the User on the website;",
          "payment by the User for the order placed on the website;",
          "processing and provision of services to the User under the terms of this Agreement.",
        ]),
      },
    ],
  },
  {
    title: <p>6. ORDER PAYMENT PROCEDURE</p>,
    point: [
      {
        text: (
          <p>
            Any transactions on the website related to the purchase of ownership
            of musical works are conducted through state-of-the-art digital
            payment services (such as Way for Pay, Apple Pay, Google Pay,
            cryptocurrencies, etc.). By using the payment service, you accept
            the terms of use of the respective payment service.
          </p>
        ),
      },
      {
        text: (
          <p>
            If funds are not received, the Platform reserves the right to cancel
            the order.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>7. INTELLECTUAL PROPERTY</p>,
    point: [
      {
        text: (
          <p>
            The Platform owns the intellectual property rights or has the
            appropriate permission of third parties in the content of the
            Website, all other trademarks, names, products and logos used on the
            Website. Such intellectual property rights may not be copied,
            imitated or used, in whole or in part, without the permission of the
            respective owner.
          </p>
        ),
      },
      {
        text: (
          <p>
            The Platform, as the owner of intellectual property rights to the
            Token posted on the Website, has, in particular, such rights as:
          </p>
        ),
      },
      {
        text: DocsTextList([
          "the right to use;",
          "the exclusive right to permit use;",
          "the exclusive right to prevent misuse, including prohibiting such use;",
          "any other intellectual property rights under applicable law.",
        ]),
      },
    ],
  },
  {
    title: <p>8. OWNERSHIP AND COPYRIGHT</p>,
    point: [
      {
        text: (
          <p>
            The User, having purchased the Token in the manner prescribed by
            this Offer, receives a single exclusive perpetual license to use the
            Token, which embodies the rights to receive a share of the Royalties
            for listening, without the right to license, commercially use,
            reproduce or distribute the Tokens contained therein.
          </p>
        ),
      },
      {
        text: (
          <p>
            The User has the right to alienate the Tokens belonging to him in
            the future on the same rights as provided for by the terms of this
            Offer and the Agreement on the Terms and Conditions of Use.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>9. RIGHTS AND OBLIGATIONS OF THE PARTIES</p>,
    point: [
      {
        text: <p>The User is obliged to:</p>,
      },
      {
        text: DocsTextList([
          "get acquainted with the information about the Services, which is posted on the website of the Platform;",
          "timely pay and receive Tokens and other services under the terms of this Agreement;",
          "The User has the right to demand that the Platform comply with the terms of this Agreement.",
        ]),
      },
      {
        text: <p>The Platform is obliged to:</p>,
      },
      {
        text: DocsTextList([
          "comply with the terms of this Agreement;",
          "transfer the Services to the User in accordance with the choice posted on the website and the terms of this Agreement;",
          "The Platform is not responsible, cannot act as a defendant in court and does not compensate for losses incurred by the User due to the action or inaction of third parties",
        ]),
      },
      {
        text: (
          <p>
            The Platform has the right to unilaterally suspend the provision of
            services under this Agreement in case of violation of the terms of
            this Agreement by the User.{" "}
          </p>
        ),
      },
    ],
  },
  {
    title: <p>10. LIABILITY OF THE PARTIES</p>,
    point: [
      {
        text: (
          <p>
            The Parties shall be liable for non-fulfillment or improper
            fulfillment of the terms of this Agreement in the manner prescribed
            by this Agreement and the current legislation of Ukraine.
          </p>
        ),
      },
      {
        text: (
          <p>
            In case of force majeure, the parties are exempt from the terms of
            this Agreement. For the purposes of this Agreement, force majeure
            circumstances shall mean events of an extraordinary, unavoidable,
            unpredictable nature that exclude or objectively impede the
            execution of this Agreement, the occurrence of which the Parties
            could not foresee and prevent by reasonable measures.
          </p>
        ),
      },
      {
        text: (
          <p>
            A party invoking force majeure circumstances must notify the other
            party in writing by e-mail of the occurrence of such circumstances
            within five calendar days.
          </p>
        ),
      },
      {
        text: (
          <p>
            The parties shall make every effort to resolve any disputes through
            negotiations. If it is impossible to reach an agreement, disputes
            shall be resolved in accordance with the current legislation of
            Ukraine.
          </p>
        ),
      },
      {
        text: (
          <p>
            Any claims (complaints), proposals, statements of the User are
            accepted at the e-mail address specified in the &#34;Contact
            Details&#34; paragraph.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>11. LIMITATION OF LIABILITY</p>,
    point: [
      {
        text: (
          <p>
            The Platform shall not be liable to the User or any third party for
            any complaints or losses that may arise from any transactions
            related to the purchase of the Token.{" "}
          </p>
        ),
      },
      {
        text: (
          <p>
            The Platform and its partners shall not be liable to the User for
            any damages, such as: loss of data, profits, income, reputational
            losses or for any other damage that cannot be foreseen.
          </p>
        ),
      },
      {
        text: (
          <p>
            The User, by accepting the terms of this Offer, is liable to the
            Platform and/or third parties for any complaints, lawsuits, damages,
            losses (actual and indirect), legal costs and other expenses arising
            from them, for:
          </p>
        ),
      },
      {
        text: DocsTextList([
          "violation of the terms of this Offer;",
          "abusing the functionality of the Website, carrying out software manipulations with the Website with or without unlawful violation of the integrity of the Website data;",
          "violation of the current legislation of any country in the world and/or the rights of third parties established by such legislation;",
          "slandering (spreading false information) about the Platform or third parties directly or indirectly related to it.",
        ]),
      },
    ],
  },
  {
    title: <p>12. DISPUTE RESOLUTION</p>,
    point: [
      {
        text: (
          <p>
            The Platform and the User resolve disagreements arising from the
            relations covered by this Offer through negotiations.
          </p>
        ),
      },
      {
        text: (
          <p>
            In case of failure to reach an agreement through negotiations, the
            dispute shall be settled in court, in accordance with the current
            legislation of Ukraine.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>13. FINAL PROVISIONS</p>,
    point: [
      {
        text: (
          <p>
            MusicDex reserves the right to unilaterally amend this Agreement
            with its prior publication on the musicdex.co website.
          </p>
        ),
      },
      {
        text: (
          <p>
            The user is responsible for the accuracy of the information
            specified when placing an order. The use of the website to view the
            Services, as well as to purchase, is free of charge for the User.
          </p>
        ),
      },
      {
        text: (
          <p>
            The information provided by the User is confidential. The Platform
            uses information about the User solely for the purposes of the
            functioning of MusicDex (sending a message to the User about the
            execution of the order, sending advertising messages, etc.).
          </p>
        ),
      },
      {
        text: (
          <p>
            This Agreement shall enter into force on the date of payment for the
            order or registration on the musicdex.co website and shall be valid
            until all the terms of the Agreement are fulfilled.
          </p>
        ),
      },
      {
        text: <p>The text of this Offer is in English.</p>,
      },
    ],
  },
];

export const pointsUA = [
  {
    title: <p>1. ВИЗНАЧЕННЯ ТЕРМІНІВ</p>,
    point: [
      {
        text: (
          <p>
            Даним пунктом визначається інтерпретація наведених нижче термінів та
            дефініцій, які використовуються у цій Оферті.
          </p>
        ),
      },
      {
        text: (
          <p>
            «Веб-сайт» – веб-сторінка, розташована на доменному імені:
            musicdex.co, включаючи сукупність всієї розміщеної на веб-сторінці
            інформації, текстів, графічних елементів, елементів дизайну,
            зображень, фото та відеоматеріалів та інших результатів
            інтелектуальної діяльності, а також інформаційних і технічних
            засобів. Термін «Веб-сайт» охоплює посилання на інші відкриті чи
            закриті веб-ресурси, що використовуються для забезпечення діяльності
            веб-сторінки;
          </p>
        ),
      },
      {
        text: (
          <p>
            «Користувач» – будь-яка фізична особа, яка має доступ до Веб-сайту
            за допомогою мережі Інтернет;
          </p>
        ),
      },
      {
        text: (
          <p>
            «NFT» – (повн. nonfungible token) - унікальний цифровий актив у
            вигляді невзаємозамінного токена;
          </p>
        ),
      },
      {
        text: (
          <p>
            «Токен» – віртуальний актив що уособлює в собі права власності на
            долю прибутку від музикального твору.
          </p>
        ),
      },
      {
        text: (
          <p>
            «Послуги» – можливість шанувальникам музики (&#34;Інвесторам&#34;) зробити
            свій внесок у фінансування виробництва відео- та аудіозаписів, а
            також заходів з просування та маркетингу записів, надаючи можливість
            шанувальникам придбати частку прав на участь у чистих доходах (права
            на участь) від пісень та/або записів у вигляді Токену.
          </p>
        ),
      },
      {
        text: (
          <p>
            Всі терміни та дефініції, не визначені цим розділом, тлумачаться у
            значенні, встановленому чинним законодавством України, а при
            відсутності такого визначення – в їх звичайному розумінні.
          </p>
        ),
      },
    ],
  },

  {
    title: <p>2. ВСТУП</p>,
    point: [
      {
        text: (
          <p>
            Дійсна Оферта у сукупності з Політикою конфіденційності та захисту
            персональних даних та іншою інформацією, розміщеною на Веб-сайті,
            визначають умови доступу Користувачів до Веб-сайту, а також умови
            використання Платформи, розміщеної на Веб-сайті.
          </p>
        ),
      },
      {
        text: (
          <p>
            Для отримання інформації щодо порядку отримання, обробки та
            зберігання персональних даних, Користувач може ознайомитись з
            Політикою конфіденційності та захисту персональних даних, розміщеної
            на Веб-сайті.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>3. УМОВИ ПРИЙНЯТТЯ ОФЕРТИ</p>,
    point: [
      {
        text: (
          <p>
            Використання Користувачем даного Веб-сайту означає автоматичне і
            повне прийняття Користувачем умов цієї Оферти.
          </p>
        ),
      },
      {
        text: (
          <p>
            У разі, якщо Користувач не згоден з умовами цієї Оферти, останній
            зобов&#39;язаний припинити використання Веб-сайту і негайно покинути
            його.
          </p>
        ),
      },
      {
        text: (
          <p>
            Прийняттям умов цієї Оферти, Користувач підтверджує, що йому
            виповнилось 18 років.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>4. ВЗАЄМОДІЯ З ПЛАТФОРМОЮ</p>,
    point: [
      {
        text: (
          <p>
            Платформа надає Користувачам можливість придбання прав власності на
            частину роялті від музичного твору виражених в Токенах, що
            виготовляються у рамках проєкту «MusicDex», та передаються
            Користувачам шляхом під’єднання його до Платформи за допомогою
            криптовалютного гаманця.
          </p>
        ),
      },
      {
        text: (
          <p>
            Усі транзакції між Платформою та Користувачами здійснюються виключно
            у мережі блокчейну Polygon.
          </p>
        ),
      },
      {
        text: (
          <p>
            У разі придбання Токену, Користувач сплачує комісію за транзакцію
            (газ), що стягується у мережі блокчейну.
          </p>
        ),
      },
      {
        text: (
          <p>
            Користувач несе усі витрати, пов’язані з придбанням ним Токену та
            отримання прибутку від роялті, зокрема, за всі податки, що виникають
            та застосовуються до Користувача, пов’язані з таким придбанням.
          </p>
        ),
      },
      {
        text: (
          <p>
            Користувач здійснює користування Платформою виключно на його власний
            ризик. Платформа не надає Користувачу жодних гарантій щодо її
            використання та придбання Токену.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>5. ПРЕДМЕТ ДОГОВОРУ</p>,
    point: [
      {
        text: (
          <p>
            Платформа зобов&#39;язується передати у власність Користувача Послуги, а
            Користувач зобов&#39;язується сплатити та прийняти Послуги на умовах
            даного Договору.
          </p>
        ),
      },
      {
        text: (
          <p>
            Цей Договір регулює купівлю-продаж послуг на Веб-сайті, в тому
            числі:
          </p>
        ),
      },
      {
        text: DocsTextList([
          "добровільний вибір Користувачем типу товарів (Токенів) на веб-сайті;",
          "самостійне оформлення Користувачем замовлення на веб-сайті; ",
          "оплату Користувачем замовлення, оформленогона веб-сайті;",
          "обробка та надання послуг Користувачеві на умовах цього Договору.",
        ]),
      },
    ],
  },
  {
    title: <p>6. ПОРЯДОК ОПЛАТИ ЗАМОВЛЕННЯ</p>,
    point: [
      {
        text: (
          <p>
            Будь-які транзакції на веб-сайті, пов’язані з купівлею частко
            власності на музичні твори проводяться через найсучасніші цифрові
            платіжні сервіси (такі як Way for Pay, Apple Pay, Google Pay,
            криптовалюти тощо). Використовуючи платіжний сервіс, ви приймаєте
            умови використання відповідного платіжного сервісу.
          </p>
        ),
      },
      {
        text: (
          <p>
            При ненадходженні коштів, Платформа залишає за собою право анулювати
            замовлення.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>7. ІНТЕЛЕКТУАЛЬНА ВЛАСНІСТЬ</p>,
    point: [
      {
        text: (
          <p>
            Платформа є власником майнових прав інтелектуальної власності або
            володіє належним дозволом третіх осіб на вміст Веб-сайту, усіх інші
            торговельних марок, назв, продуктів і логотипів, що використовуються
            на Веб-сайті. Такі об’єкти права інтелектуальної власності не можуть
            бути скопійовані, імітовані або використані, повністю чи частково,
            без дозволу відповідного власника.
          </p>
        ),
      },
      {
        text: (
          <p>
            Платформа, як власник майнових прав інтелектуальної власності на
            Токен, що розміщені на Веб-сайті, володіє, зокрема, такими правами,
            як:
          </p>
        ),
      },
      {
        text: DocsTextList([
          "право на використання;",
          "виключне право дозволяти використання;",
          "виключне право запобігати неправомірному використанню, включаючи заборону такого використання;",
          "будь-які інші права інтелектуальної власності відповідно до чинного законодавства.",
        ]),
      },
    ],
  },
  {
    title: <p>8. ПРАВО ВЛАСНОСТІ ТА АВТОРСЬКЕ ПРАВО</p>,
    point: [
      {
        text: (
          <p>
            Користувач, придбавши Токен у порядку, передбаченому цією Офертою,
            отримує одиничну виключну безстрокову ліцензію на використання
            Токену, що уособлює в собі права на отримання частки від Роялті за
            прослуховування, без права ліцензувати, комерційно використовувати,
            відтворювати або розповсюджувати Токени, що містяться в ньому.
          </p>
        ),
      },
      {
        text: (
          <p>
            Користувач має право відчужувати у подальшому належні йому Токени на
            тих самих правах, що передбачені умовами цієї Оферти та Договором
            про Умови і правила користування.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>9. ПРАВА ТА ОБОВ’ЯЗКИ СТОРІН</p>,
    point: [
      {
        text: <p>Користувач зобов&#39;язаний:</p>,
      },
      {
        text: DocsTextList([
          "ознайомитись з інформацією про Послуги, яка розміщена на сайті Платформи;",
          "своєчасно оплатити та отримати Токени та інші послуги на умовах цього Договору;",
          "Користувач має право вимагати від Платформи дотримання умов цього Договору.",
        ]),
      },
      {
        text: <p>Платформа зобов&#39;язана:</p>,
      },
      {
        text: DocsTextList([
          "дотримувати умов цього Договору;",
          "передати Користувачу Послуги у відповідності з вибором розміщеним на сайті та та умовами цього договору;",
          "Платформа не несе відповідальності, не може виступати в якості відповідача у суді і не відшкодовує збитки, що виникли у Користувача через дію чи бездіяльність третіх осіб",
        ]),
      },
      {
        text: (
          <p>
            Платформа має право: в односторонньому порядку призупинити надання
            послуг за цим Договором у випадку порушення Користувачем умов цього
            Договору.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>10. ВІДПОВІДАЛЬНІСТЬ СТОРІН</p>,
    point: [
      {
        text: (
          <p>
            Сторони відповідають за невиконання або неналежне виконання умов
            цього Договору в порядку, передбаченому цим Договором і чинним
            законодавством України.
          </p>
        ),
      },
      {
        text: (
          <p>
            У випадку обставин непереборної сили, сторони звільняються від
            виконання умов цього Договору. Під обставинами непереборної сили для
            цілей цього Договору розуміються події, що мають надзвичайний,
            невідворотний непередбачуваний характер, які виключають або
            об&#39;єктивно перешкоджають виконанню цього Договору, настання яких
            Сторони не могли передбачити й запобігти розумними мірами.
          </p>
        ),
      },
      {
        text: (
          <p>
            Сторона, що посилається на дію обставин непереборної сили повинна
            протягом п&#39;яти календарних днів у письмовому вигляді за допомогою
            електронної пошти повідомити іншу сторону про настання таких
            обставин.
          </p>
        ),
      },
      {
        text: (
          <p>
            Сторони докладають максимальних зусиль для вирішення будь-яких
            спорів шляхом переговорів. У випадку неможливості досягнення згоди,
            спори підлягають вирішенню відповідно чинного законодавства України.
          </p>
        ),
      },
      {
        text: (
          <p>
            Будь-які претензії (рекламації), пропозиції, заяви Користувача
            приймаються за адресою електронної пошти, вказаної в пункті
            «Контактні дані».
          </p>
        ),
      },
    ],
  },
  {
    title: <p>11. ОБМЕЖЕННЯ ВІДПОВІДАЛЬНІСТЬ</p>,
    point: [
      {
        text: (
          <p>
            Платформа не несе відповідальності перед Користувачем або будь-якою
            третьою особою за будь-які скарги або збитки, які можуть виникнути в
            результаті будь-яких транзакцій, пов’язаних із придбанням Токену.
          </p>
        ),
      },
      {
        text: (
          <p>
            Платформа та її партнери не несуть відповідальності перед
            Користувачем за будь-які збитки, такі як: втрата даних, прибутку,
            доходу, репутаційні втрати або за будь-які інші збитки, які
            неможливо передбачити.
          </p>
        ),
      },
      {
        text: (
          <p>
            Користувач, приймаючи умови цієї Оферти, несе відповідальність перед
            Платформою та/або третіми особами за будь-які скарги, позови, шкоду,
            збитки (фактичні та непрямі), судових витрат та інших витрат, що
            випливають з них, за:
          </p>
        ),
      },
      {
        text: DocsTextList([
          "порушення умов цієї Оферти;",
          "зловживання функціоналом Веб-сайту, здійснення програмних маніпуляцій з Веб-сайтом з протиправним порушенням цілісності даних Веб-сайту або без такого;",
          "порушення чинного законодавства будь-якої країни світу та/або прав третіх осіб, встановлених таким законодавством;",
          "здійснення наклепу (поширення неправдивих відомостей) щодо Платформи або третіх осіб, що прямо або опосередковано пов’язані з нею.",
        ]),
      },
    ],
  },
  {
    title: <p>12. ВИРІШЕННЯ СПОРІВ</p>,
    point: [
      {
        text: (
          <p>
            Платформа і Користувач вирішують розбіжності, що виникають з
            відносин, на які поширюється дана Оферта, шляхом переговорів.
          </p>
        ),
      },
      {
        text: (
          <p>
            У разі недосягнення згоди шляхом переговорів, спір підлягає
            врегулюванню у судовому порядку, відповідно до чинного законодавства
            України.
          </p>
        ),
      },
    ],
  },
  {
    title: <p>13. ПРИКІНЦЕВІ ПОЛОЖЕННЯ</p>,
    point: [
      {
        text: (
          <p>
            MusicDex залишає за собою право в односторонньому порядку вносити
            зміни в цей Договір з попередньою публікацією його на сайті
            «musicdex.co».
          </p>
        ),
      },
      {
        text: (
          <p>
            Користувач несе відповідальність за достовірність вказаної при
            оформленні замовлення інформації. Використання веб-сайту для
            перегляду Послуг, а також для придбання є для Користувача
            безоплатним.
          </p>
        ),
      },
      {
        text: (
          <p>
            Інформація, що надається Користувачем є конфіденційною. Платформа
            використовує інформацію про Користувача виключно в цілях
            функціонування MusicDex (відправлення повідомлення Користувачеві про
            виконання замовлення, надсилання рекламних повідомлень і т.д.).
          </p>
        ),
      },
      {
        text: (
          <p>
            Цей договір набирає чинності з дня оплати замовлення або реєстрації
            на веб-сайті musicdex.co і діє до виконання всіх умов Договору.
          </p>
        ),
      },
      {
        text: <p>Текст цієї Оферти викладено українською мовою.</p>,
      },
    ],
  },
];
