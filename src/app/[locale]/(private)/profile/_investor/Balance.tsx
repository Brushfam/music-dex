...
import {
  getBalance,
  getInvestBalance,
} from "@/services/users/investors/investors";
...
export const Balance = () => {
  ...
  const [investBalance, setInvestBalance] = useState<any>({});
  ...
  const handleGetBalances = useCallback(() => {
    firebaseAuth.onAuthStateChanged(async (user) => {
      if (user) {
        ...
        getInvestBalance(token)
          .then((res) => {
            setInvestBalance(res.data);
          })
          .catch((error) => {
            console.log(error);
          })
